    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-5" style={{ backgroundColor: '#09090b' }}>
        ...
        <button onClick={() => router.push('/')}
          className="px-8 py-3 border border-[rgba(201,169,110,0.4)] text-[10px] tracking-[0.22em] uppercase text-[#C9A96E] hover:bg-[#C9A96E] hover:text-black transition-all duration-300 cursor-pointer bg-transparent">
          Quay Lại Mua Sắm
        </button>
      </div>
    );
    ...
    <div className="min-h-screen pt-20 md:pt-24 pb-20 px-5 sm:px-8" style={{ backgroundColor: '#09090b' }}>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 lg:gap-16 items-start">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-10">
          ...
          {/* Contact */}
          <fieldset className="space-y-6 pb-10" style={{ borderBottom: '1px solid rgba(245,240,232,0.06)' }}>
            <legend className="text-[10px] tracking-[0.25em] uppercase text-white/40 mb-6 block">
              Thông Tin Liên Hệ
            </legend>
          ...
          <fieldset className="space-y-6 pb-10" style={{ borderBottom: '1px solid rgba(245,240,232,0.06)' }}>
            ...
            <div>
              ...
              >
                {PROVINCES.map(p => (
                  <option key={p} value={p} style={{ background: '#09090b' }}>{p}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <textarea
                {...getInputProps('note')}
                placeholder="Giao trước 6pm, gọi trước khi đến..."
                rows={3}
            ...
            <div className="space-y-3">
              {PAYMENT_OPTIONS.map(opt => (
                <label key={opt.value}
                  ...
                  style={{
                    borderColor: paymentMethod === opt.value ? 'rgba(201,169,110,0.5)' : 'rgba(245,240,232,0.08)',
                    background:  paymentMethod === opt.value ? 'rgba(201,169,110,0.04)' : 'transparent',
                  }}>
                  <input type="radio" name="payment" value={opt.value} checked={paymentMethod === opt.value}
                    onChange={() => setPayment(opt.value)} className="sr-only" />
                  <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${paymentMethod === opt.value ? 'border-[#C9A96E]' : 'border-white/20'}`}>
          ...
          <button type="submit" disabled={loading}
            className="w-full py-4 text-[10px] tracking-[0.28em] uppercase border border-[#C9A96E] text-[#C9A96E] bg-transparent cursor-pointer transition-all duration-300 relative overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed">
            <span className="absolute inset-0 bg-[#C9A96E] -translate-x-full group-hover:translate-x-0 group-disabled:translate-x-[-101%] transition-transform duration-400" />
            <span className="relative z-10 group-hover:text-black group-disabled:text-[#C9A96E] transition-colors duration-400 flex items-center justify-center gap-3">
              {loading ? (
                <>
        ...
        <div className="lg:sticky lg:top-24">
          <p className="text-[10px] tracking-[0.25em] uppercase text-white/40 mb-6">Đơn Hàng Của Bạn</p>

          {/* Items list */}
          <div className="space-y-3 pb-5 mb-5" style={{ borderBottom: '1px solid rgba(245,240,232,0.06)' }}>
            {cart.map(item => (
              <div key={item._id} className="flex gap-3 items-center">
                <div className="relative w-12 h-12 flex-shrink-0 overflow-hidden"
                  ...
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#C9A96E] text-black text-[8px] rounded-full flex items-center justify-center font-bold">
                    {item.quantity}
                  </span>
                </div>
                ...
                <span className="font-['Cormorant_Garamond'] text-[15px] text-white/60 flex-shrink-0">
          ...
          {/* Totals */}
          <div className="space-y-2.5 pb-5 mb-5" style={{ borderBottom: '1px solid rgba(245,240,232,0.06)' }}>
          ...
          <div className="p-4 text-[11px] text-white/30 leading-[1.8]"
            ...
            📧 Email xác nhận sẽ được gửi sau khi đặt hàng
          </div>
        </div>
      </div>
    </div>
